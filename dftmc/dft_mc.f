...
81        sita=2.0 * pi *ran2(idum)
		...
		if(i-2.le.0) then 
	       ...
	       confg(ik,i,1)=confg(ik,i-1,1)+cos(sita)*sin(fai)
	       confg(ik,i,2)=confg(ik,i-1,2)+sin(sita)*sin(fai)
	       confg(ik,i,3)=confg(ik,i-1,3)+cos(fai)
	       
	  	else 

             xix=confg(ik,i-1,1)+cos(sita)*sin(fai)
	       yiy=confg(ik,i-1,2)+sin(sita)*sin(fai)
	       ziz=confg(ik,i-1,3)+cos(fai)
...
10     continue
	 ...
	 do 100 j=1,numb
	  ...
	  zz1=confg(j,1,3)*2.0+zrz
	  zz2=confg(j,2,3)*2.0+zrz
	  zz3=confg(j,3,3)*2.0+zrz
...
!        coordinates transformation 
          zz2=zrz
		zz1=(confg(j,1,3)-confg(j,2,3))*2.0+zrz
		zz3=(confg(j,3,3)-confg(j,2,3))*2.0+zrz
...
!         endif
          ...
          zz3=zrz
		zz1=(confg(j,1,3)-confg(j,3,3))*2.0+zrz
		zz2=(confg(j,2,3)-confg(j,3,3))*2.0+zrz
...
100    continue
	 ...
	 ic2=ic2+ict2(i)
	 ic3=ic3+ict3(i)
!	 write(*,*)i, 'ic1=', ic1, 'ic2=',ic2,'ic3=',ic3
200     continue
