!   ****************************************************************
!    * A combination of DFT and MC simulaiton for claculation of  *
!    * microstructure of semi-flexible polymeric fluids           *
!    *               Updated version 7....                        *
!!   *                                    Author:Dapeng Cao       *
!    *                                      April 26,2004         *
!   ****************************************************************
...
!		
	...
	PARAMETER (M=4,NZ=401,numb=50000)
	REAL RHO(NZ),RHO1(NZ),RD(NZ),ros(m,2*nz),rocm(2*nz-1) 
c	DOUBLE PRECISION FL(M,2*NZ),FR(M,2*NZ),FF(2*NZ)
      real  confg(numb,m,3),rend(2*nz-1),ang_num(2*nz-1)
...
!		  randm1=ran2(idum)
!	      cta0=(pi-cct)*randm1
!	      engr1=eslg_star*(1.0-cos(cta0))

!	      randm2=ran2(idum)
	    
!		  if(engr1.gt.randm2) then
!	      cta0=0.0
!	      else
!	      inumb=inumb+1
!	      endif
	      ...
	      elseif(idice.eq.2) then
	       ...
	       xplus=xvi
	       yplus=yvi*cos(cta0)+zvi*sin(cta0)
	       zplus=yvi*(-sin(cta0))+zvi*cos(cta0)

	       goto 100

90           xplus=cos(cta0)*xvi-sin(cta0)*zvi
             yplus=yvi
		   zplus=sin(cta0)*xvi+cos(cta0)*zvi
		   
		   goto 100

95           xplus=xvi*cos(cta0)+yvi*sin(cta0)
             yplus=yvi*cos(cta0)-xvi*sin(cta0)
		   zplus=zvi
		   		   		  
100          confg(ik,i,1)=confg(ik,i-1,1)+xplus
...
!  calculate the reduced chemical potential PER SEGMENT in the bulk phase, AMU
	CALL CPS(M,ETA,AMU,bl)
!	WRITE(*,*) AMU*FLOAT(M)
 	  
	write(*,*) '.....start DFT iteration....inumb=',inumb
	write(*,*)'eta=',eta,'ross=',ross,'zh=',zh,'cta0=',cta0/pi,'pi'

!  calculate the recurrence functions and the effective external potential 

1	  call aver(m,nz,dz,amu,ros,rho,rocm,confg,rend,ang_num,bl)
...
c  the average total packing fraction inside the pore
	SUMP=(RHO(1)+RHO(NZ))/2
	cmsm=(rocm(1)+rocm(nz))/2.0
	DO I=2,NZ-1
	SUMP=SUMP+RHO(I)
	cmsm=cmsm+rocm(i)
	ENDDO
	roav_cm=cmsm/float(nz-1)
...
!	RHO(I)=RHO(I)/ROSS
	...
	open(4,file='ros.dat')
	write(4,51)y,m*ros(1,i)/roav,m*ros(m/2+1,i)/roav
	
      open(5,file='rocm1.dat')
	write(5,52)y,rocm(i)/roav_cm,rend(i),ang_num(i)

51	FORMAT(1X,F8.4,4X,2F10.6)
52    format(1x,f8.4,4x,3f10.6)	
53    format(1x,f8.4,f12.6) 
50	CONTINUE
	CLOSE(3)
...
*   ================= subroutine program =======================
	...
	subroutine aver(m,nz,dz,amu,ros,rho,rocm,confg,rend,ang_num,bl)
	 parameter(numb=50000)
	 real confg(numb,m,3),ros(m,2*nz-1),rocm(2*nz-1),rend(2*nz-1)
	 real ff(2*nz-1),rho(nz),zz(m),cmsum(2*nz-1),cz(m),ang_num(2*nz-1)
	 ...
	 Do 10 i=1,2*nz-2
	  z1=dz*float(i-1)
	  call sfm1d(m,dz,nz,z1,sum1,rho,bl)
	  ff(i)=exp(-sum1+amu)
!	  write(*,*) i, '  ', ff(i)
10     continue
	 ...
	 do 100 j=1,numb
	   ...
	   do 50 im=1,m
	      ...
	      do kj=1,m
		   ...
		   zz(kj)=(confg(j,kj,3)-confg(j,im,3))*2.0*bl+zrz
...
50	 continue
	    ...
	    do kj=1,m
	    cz(kj)=(confg(j,kj,3)-cm)*2.0*bl+zrz
	    icz(kj)=int(cz(kj)/dz)+1

	    if(icz(kj).le.0.or.icz(kj).ge.2*nz-2) goto 100
	    enddo

	    cfsum=1.0
	    do kk=1,m
	    cfsum=cfsum*ff(icz(kk))
	    enddo
	    cmsum(i)=cmsum(i)+cfsum
            icount=icount+1
            cmx=bl*2.0*(confg(j,m,1)-confg(j,1,1))
            cmy=bl*2.0*(confg(j,m,2)-confg(j,1,2))
            cmz=bl*2.0*(confg(j,m,3)-confg(j,1,3))
...
30	CONTINUE
	...
	SUBROUTINE SFM1D(M,DZ,NZ,Z,SUM1,RHO,bl)
...
!    n2, n3, nv2
	CALL FMN1D(M,DZ,NZ,Z1,DFN,DFA,RHO,bl)
...
!	WRITE(*,*) Z,SUM1
	...
	SUBROUTINE FMN1D(M,DZ,NZ,Z1,DFN,DFA,RHO,bl)
...
!	see Eq.(18) in JCP, V117,10156(2002). 
	...
	GS11=1./(1.-AN3)
     &    +bl*0.5*AN2*XI1/(1.-AN3)**2
     &    +bl**2*AN2**2*XI1/(18.*(1.-AN3)**3)

C	WRITE(*,*) GS11
!	derivatives of GS11 wrt n2, n3 and nv2
	DGN2=bl*0.5*(2.-XI1)/(1.-AN3)**2+AN2*bl**2/(1.-AN3)**3/9.0
	DGN3=1./(1.-AN3)**2+bl*AN2*XI1/(1.-AN3)**3+bl*bl*AN2**2*XI1
     &      /(1.-AN3)**4/6.0
	DGNV2=-bl*ANV2I/AN2/(1.-AN3)**2-bl**2*ANV2I/(1.-AN3)**3/9.0
...
!	hard-sphere term from the Carnahan-Starling Equation state
	AMHS=ETA0*(8.-9.*ETA0+3.*ETA0**2)/(1.-ETA0)**3
	 rn2=4.*pi*ross
!	chain connectivity
C	GHS=(1.-0.5*ETA)/(1.-ETA)**3
C	DGHS=3.*ETA/(1.-ETA)-ETA/(2.-ETA)
	Ghs=1/(1-eta0)+rn2*0.5*bl/(1-eta0)**2+bl**2*rn2**2/(1-eta0)**3/18.
	dghs1=4*pi*(bl*0.5/(1-eta0)**2+rn2*bl*bl/(1-eta0)**3/9.)
	dghs2=1/(1-eta0)**2+rn2*bl/(1-eta0)**3+rn2**2*bl**2/(1-eta0)**4/6.
	dghs2=dghs2*4.*pi/3
	dghs=ross*(dghs1+dghs2)/ghs
