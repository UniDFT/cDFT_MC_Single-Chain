...
!	real ang_num(2*nz-1), rend(2*nz-1)
	...
	Sigma=3.93
     	...
     	Diam=sigma*(1+0.2977*TF)/(1.+0.33163*TF+0.0010477*TF**2)	 
	rad=sigma/2.0
     	Vfuse=(m*pi/3.0)*(rad+bl/2.)**2*(sigma-bl/2.)
	vfuse=vfuse-(m-2)*pi*(rad-bl/2.)**2*(sigma+bl/2)/3.
     	vtang=m*pi*sigma**3/6.0
	deff=((6.0*vfuse)/m/pi)**(1/3.)
		
	write(*,*) 'vf=', vfuse, 'vt=', vtang, vfuse/vtang, deff	      
...
1	Call aver(m,ma,eta,nz,dz,ros,rho,rocm,confg,diam,deff,sigma)
...
41	FORMAT(4X,F8.4,4X,F10.4,4X,F10.4)
40	CONTINUE
	CLOSE(2)
	ENDIF
...
c  the average total packing fraction inside the pore
	...
	ENDDO
	SUMP(J)=SUMP(J)*(deff**3)*PI/6./FLOAT(NZ-1)
50	CONTINUE
	   ...
	   confg(ik,1,3)=0.0
	    ...
	    Do 80 i=2,m
	 	...
	 	if(i-2.le.0.or.irigid.eq.0) then 
		  ...
		  randm1=ran2(idum)
	      randm2=ran2(idum)
	      cta0=Bangle+pi*(randm1-0.5)/2.0
!121	      engr1=exp(-eslg_star*(1.0+cos(pi-cta0)))
121		  engr1=exp(-eslg_star*(cta0-Bangle)**2)	          
		  if(engr1.lt.randm2) then
			...
			if(randm2.gt.0.95) then
	         ...
	         goto 122
              ...
              cta0=bangle+(cta0-bangle)*ran2(idum)
	        goto 121
	      endif
...
99	 call MAK_RAS_SAD(confg,m)
	...
	subroutine aver(m,ma,eta,nz,dz,ros,rho,rocm,confg,diam,deff,sigma)
...
!	 real ang_num(2*nz-1),rend(2*nz-1)
	 ...
	 Do 10 i=1,nz
	  z1=dz*float(i-1)
	  call sfm1d(m,ma,eta,dz,nz,z1,sum1,rho,diam,deff,sigma)
	 ...
	 do 100 j=1,numb
	   ...
	   do 50 im=1,m
	      ...
	      do kj=1,m
		   ...
		   zz(kj)=(confg(j,kj,3)-confg(j,im,3))*deff+zrz
...
50	 continue
	    ...
	    do kj=1,m
	    cz(kj)=(confg(j,kj,3)-cm)*deff+zrz
...
! SUBROUTINES USED: SFM1D(M,MA,ETA,DZ,NZ,Z1,SUM1,RHO), SELF-CONSISTENT MEAN-FIELD AT Z1
	...
	REAL FL(M,NZ),FR(M,NZ),FF(M,NZ),deff,sigma
	deff=3.3
	sigma=3.9
	DO 10 I=1,NZ
	FL(1,I)=1.0
...
50	CONTINUE
	...
	SUBROUTINE SFM1D(M,MA,ETA,DZ,NZ,Z,SUM1,RHO,diam,deff,sigma)
...
!      write(*,*) 'sum1(1)=',sum1(1),sum1(2)
	...
	SUBROUTINE CPEHSC(M,DZ,NZ,Z,RHO,CPHSC, diam,deff) 
	IMPLICIT NONE
	INTEGER M,NP,NZ,I
	REAL PI,SF21,SF31,SF21VI,DF21,DF31,DF21VI,Z,Z1,DZ,CPHSC
	REAL DFN(3),DFA(3),RHO(2,NZ),RHOT(NZ),diam, deff
...
!    n2, n3, nv2
	CALL FMN1D(M,DZ,NZ,Z1,DFN,DFA,RHOT,diam,deff)
...
10	CONTINUE
	...
	SUBROUTINE FMN1D(M,DZ,NZ,Z1,DFN,DFA,RHOT,diam,deff)
...
!  with respect to the weighted densities at position z1 given a density profile rho(z)
	...
	REAL XI2,XI1,F31,F21VI,DFN3,DGN2,diam,deff
...
! impose the boundary conditions and system symmetry 
	IF ((IZ2.GT.(2*NZ-1-nint(diam/2./dz))).OR.
	&           (IZ2.LT.1+nint(diam/2./dz))) THEN
	F21=0.0
...
!	see Eq.(18) in JCP, V117,10156(2002). 
	...
	GS11=1./(1.-AN3)
     &    +0.25*deff*AN2*XI1/(1.-AN3)**2
     &    +AN2**2*deff**2*XI1/(72.*(1.-AN3)**3)

!	derivatives of GS11 wrt n2, n3 and nv2
	DGN2=0.25*deff*(2.-XI1)/(1.-AN3)**2+AN2*deff**2/(1.-AN3)**3/36.0
	DGN3=1./(1.-AN3)**2+0.5*deff*AN2*XI1/(1.-AN3)**3
     &       +AN2**2*deff**2*XI1/(1.-AN3)**4/24.0
	DGNV2=-0.5*deff*ANV2I/AN2/(1.-AN3)**2
     &      -deff**2*ANV2I/(1.-AN3)**3/36.0

	ARR=(1./FLOAT(M)-1.)/PI/deff**2
...
!	 The following code is for 10-4-3 Steele's potential
...
!    	   ENDDO
         ...
         eu12(1)=eu12(1)*2.0*pi*rowall*sigfw**3/tmp
	   eu12(2)=eu12(2)*2.0*pi*rowall*sigfw**3/tmp

	   If(Ma.eq.2) then
	    ATemp1=0.4*(sigfw/z)**9-(sigfw/z)**3
     	    ATemp2=0.4*(sigfw/z2)**9-(sigfw/z2)**3
