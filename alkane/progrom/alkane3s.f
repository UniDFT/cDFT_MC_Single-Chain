...
C***************************************************************
	...
	PARAMETER (PI=3.141592654,numb=50000)
	PARAMETER (ICODE=0,M=4,MA=2,NZ=551,DZ=0.02,F=0.02)
	REAL RHO(2,NZ),ROA,ROB,RHO1(2,NZ),RD(NZ),SUMP(2)
	REAL confg(numb,m,3),ros(m,2*nz-1)
	real rocm(2*nz-1),ang_num(2*nz-1), rend(2*nz-1)
...
1	Call aver(m,ma,eta,nz,dz,ros,rho,rocm,confg,rend,ang_num,diam)
...
99	 call MAK_RAS_SAD(confg,m)
	 ...
	 subroutine aver(m,ma,eta,nz,dz,ros,rho,
	&                     rocm,confg,rend,ang_num,diam)
	 parameter(numb=50000)
	 real confg(numb,m,3),ros(m,2*nz-1),rocm(2*nz-1),rend(2*nz-1)
	 real sum1(m),ang_num(2*nz-1),diam
