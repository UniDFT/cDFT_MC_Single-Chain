...
!     DIBLOCK SQUARE-WELL CHAINS CONFINED IN AN ATTRACTIVE SLIT-LIKE PORE
!						(THE SEGMENTS HAVE THE SAME SIZE)
!	KEY PARAMETERS

!	DZ		step length in the units of hard-sphere radius  
!	ZH		the pore width in the units of hard-sphere radius 
...
!	ROA/ROB	reduced number density of segment A/B, RHO*SIGMA**3/8=rho*radius**3
!	ETA		packing fraction of segments  
!	RHO(2,I)	reduced density profiles for A and B segments in the block,rhoB*radius**3    
...
!     ICODE=0, intialize calculate; ICODE=1, continue calculation
!		
! SUBROUTINES USED: FFLR(M,MA,ETA,NZ,DZ,FL,FR,FF,RHO), SELF-CONSISTENT FIELD	
!	
	IMPLICIT NONE
	INTEGER I,J,K,M,MA,NZ,ICODE,ITER
	REAL PI,DZ,XB(2),F,ZH,ETA,Y,SADD
	PARAMETER (PI=3.141592654)
	PARAMETER (ICODE=0,M=4,MA=2,NZ=1001,DZ=0.02,F=0.005)
...
1	CALL FFLR(M,MA,ETA,NZ,DZ,FL,FR,FF,RHO)
...
C	PAUSE
	...
	OPEN(2,FILE='blk.ini')
	DO 40 I=1,NZ
	Y=DZ*FLOAT(I-1)/2.		! in units of diameter (2)
...
c  the average total packing fraction inside the pore
	...
	ENDDO
	SUMP(J)=SUMP(J)*8.*PI/6./FLOAT(NZ-1)
50	CONTINUE
...
! output results
	...
	Y=DZ*FLOAT(I-1)/2		! Y is in the units of DIAMETER!
...
60	CONTINUE
	...
	SUBROUTINE FFLR(M,MA,ETA,NZ,DZ,FL,FR,FF,RHO)
...
! SUBROUTINES USED: SFM1D(M,MA,ETA,DZ,NZ,Z1,SUM1,RHO), SELF-CONSISTENT MEAN-FIELD AT Z1
	...
	REAL DZ,RHO(2,NZ),Z1,SUM1(M),S,ETA
	REAL FL(M,NZ),FR(M,NZ),FF(M,NZ)
	...
	FR(M,I)=1.0
	Z1=DZ*FLOAT(I-1)
	CALL SFM1D(M,MA,ETA,DZ,NZ,Z1,SUM1,RHO)
	DO J=1,M
	FF(J,I)=EXP(-SUM1(J))
	ENDDO
10    CONTINUE
...
50	CONTINUE
	...
	SUBROUTINE SFM1D(M,MA,ETA,DZ,NZ,Z,SUM1,RHO)
...
!  calculate the excess chemical potential due to chain connectivity and hard-sphere interactions 
	CALL CPEHSC(M,DZ,NZ,Z,RHO,CPHSC)
...
! overall density field
	...
	SUBROUTINE CPEHSC(M,DZ,NZ,Z,RHO,CPHSC) 
	IMPLICIT NONE
	INTEGER M,NP,NZ,I
	REAL PI,SF21,SF31,SF21VI,DF21,DF31,DF21VI,Z,Z1,DZ,CPHSC
	REAL DFN(3),DFA(3),RHO(2,NZ),RHOT(NZ)
	PARAMETER (PI=3.141592654)
!  calculate the excess chemical potential due to chain connectivity and hard-sphere interactions 
!  overall density
!	SUBROUTNES USED:
...
!    n2, n3, nv2
	CALL FMN1D(M,DZ,NZ,Z1,DFN,DFA,RHOT)
...
10	CONTINUE
	...
	SUBROUTINE FMN1D(M,DZ,NZ,Z1,DFN,DFA,RHOT)
...
!  with respect to the weighted densities at position z1 given a density profile rho(z)
	...
	REAL XI2,XI1,F31,F21VI,DFN3,DGN2
...
!	lamda,sigma,eps are square-well parameters
	...
	IMPLICIT NONE
	REAL VDWA,PI,PI4,ROSS,ETA,AMR,AMHS,GHS,DGHS,AMC,AMVDW,AMU
	INTEGER	I,J,M,MA
...
! wall potential, wall: enregy,kT; wlam: attraction distance, in units of radius 
	...
	DATA EPS/0.0,0.,0.,0.0/
	DATA SIGMA/2.0,2.0,2.0,2.0/
	END
