...
!   ****************************************************************
      ...
      PROGRAM Thetered_DFT_MC_Apr_27

!		
	PARAMETER (PI=3.141592654)
	PARAMETER (M=50,NZ=401,numb=10000)
...
1	  call aver(m,nz,dz,amu,ros,rho,rocm,confg,rend,ang_num)
...
*   ================= subroutine program =======================
	...
	subroutine aver(m,nz,dz,amu,ros,rho,rocm,confg,rend,ang_num)
	 ...
	 do 100 j=1,numb
	   ...
	   do 50 im=1,m
	      ...
	      do kj=1,m
		   ...
		   zz(kj)=confg(j,kj,3)*2.0
	       iz(kj)=int(zz(kj)/dz)+1
	   
	       if(kj.eq.im) then
		     if(iz(kj).ne.i) goto 50   
	       endif
...
50	 continue
	    ...
	    cmsum(i)=cmsum(i)+cfsum
            icount=icount+1
            cmx=2.0*(confg(j,m,1)-confg(j,1,1))
            cmy=2.0*(confg(j,m,2)-confg(j,1,2))
            cmz=2.0*(confg(j,m,3)-confg(j,1,3))
